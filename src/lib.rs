...
use frame_support::{
    ...
    traits::fungibles::Inspect, traits::fungibles::Mutate, traits::fungibles::Transfer,
...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn add_liquidity(
            ...
            // Check inputs
                ...
                T::exists(asset_0) && T::exists(asset_1),
    ...
    impl<T: Config> traits::Swap<T::AccountId> for Pallet<T> {
        ...
        fn swap(
            ...
            // Check inputs
                ...
                Self::balance(asset, &buyer) >= amount,
                Error::<T>::InsufficientBalance
    ...
    pub struct GenesisConfig<T: Config> {
        ...
        pub liquidity_pools: Vec<(
            (BalanceOf<T>, AssetIdOf<T>),
            (BalanceOf<T>, AssetIdOf<T>),
            AccountIdOf<T>,
        )>,
    }
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            for (amount_0, amount_1, liquidity_provider) in &self.liquidity_pools {
                ...
                assert!(
                    pair.0.value > <BalanceOf<T>>::default(),
                    "Amount should not be zero"
                );
                assert!(
                    pair.1.value > <BalanceOf<T>>::default(),
                    "Amount should not be zero"
                );

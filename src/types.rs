...
impl<T: Config> LiquidityPool<T> {
    ...
    fn create(pair: (AssetIdOf<T>, AssetIdOf<T>)) -> Result<AssetIdOf<T>, DispatchError> {
        ...
        T::Assets::create(
            ...
            dex.clone(),
            true,
            T::LiquidityPoolTokenMinimumBalance::get(),
        )?;
    ...
    fn calculate_price(&self, amount: (BalanceOf<T>, AssetIdOf<T>)) -> BalanceOf<T> {
        ...
        let other = if asset == self.pair.0 {
            self.pair.1
        } else {
            self.pair.0
        };
...
mod tests {
    ...
    #[test]
    fn liquidity_pool() {
        let _pool = LiquidityPool::<Test> {
            id: u32::MAX,
            pair: (ASSET_0, ASSET_1),
            account: 16254321,
        };
    }
    ...
    fn adds_liquidity() {
        new_test_ext().execute_with(|| {
            ...
            // Check prices
            assert_eq!(
                <LiquidityPool<Test>>::calculate(1 * UNITS, 10 * UNITS, 500 * UNITS),
                45330
            );
    ...
    fn swaps_with_native_currency() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                Assets::balance(ASSET_1, pool.account),
                (500 * UNITS) - 249624
            );
            assert_eq!(Assets::total_issuance(pool.id), 10 * UNITS);
        });
